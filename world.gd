...
func _process(_delta: float) -> void:
    var m = portal_two.global_transform * (portal_one.global_transform.inverse() * player.cam.global_transform).rotated(Vector3.UP, PI)
    cam_one.global_transform = m

    var n = portal_one.global_transform * (portal_two.global_transform.inverse() * player.cam.global_transform).rotated(Vector3.UP, PI)
